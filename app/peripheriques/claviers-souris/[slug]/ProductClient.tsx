...
export default function ClavierSourisClient({ slug }: { slug: string }) {
	const id = slug;
							...
							<div className="space-y-3">
								{prixEnseignes.map((enseigne, index) => (
									...
									>
										...
										<div className="flex items-center gap-3">
											<span className="text-base xs:text-xl sm:text-2xl font-bold text-(--prim)">{formatPrix(enseigne.prix)}</span>
					...
					<div className="space-y-6">
						<div>
							<h3 className="font-semibold text-lg mb-2">
								{isClavier
									? `Quel type de switch choisir pour un clavier de télétravail comme le ${item.name} ?`
									: `Pourquoi choisir une souris ergonomique comme la ${item.name} pour le télétravail ?`}
							</h3>
							<p className="text-gray-700">
								{isClavier
									? `Le ${item.name} utilise des switchs ${(item as Clavier).features.typeSwitch} avec une force d\u0027actuation de ${(item as Clavier).features.forceActuation}. Pour le télétravail, les switchs silencieux et à faible résistance réduisent la fatigue lors de longues sessions de frappe. Un clavier confortable améliore votre productivité et préserve vos articulations au quotidien.`
									: `La ${item.name} offre une ergonomie ${(item as Souris).features.ergonomie} avec un capteur ${(item as Souris).features.typeCapteur}. Une souris ergonomique réduit les tensions au niveau du poignet et de l\u0027avant-bras, essentielles pour les longues journées de travail à domicile. Son design adapté prévient les troubles musculosquelettiques liés à l\u0027utilisation prolongée.`}
							</p>
						</div>
						<div>
							<h3 className="font-semibold text-lg mb-2">
								{isClavier
									? `Le ${item.name} est-il compatible multi-appareils ?`
									: `La ${item.name} fonctionne-t-elle sur plusieurs appareils ?`}
							</h3>
							<p className="text-gray-700">
								{isClavier
									? `Ce clavier propose une connectivité ${(item as Clavier).features.connectivite} avec le multi-devices : ${(item as Clavier).features.multiDevices}. La compatibilité ${(item as Clavier).features.compatibilite} vous permet de basculer facilement entre votre ordinateur professionnel et personnel, un atout majeur en télétravail.`
									: `Cette souris dispose d\u0027une connectivité ${(item as Souris).features.connectivite} avec le multi-devices : ${(item as Souris).features.multiDevices}. La compatibilité ${(item as Souris).features.compatibilite} permet de l\u0027utiliser sur tous vos appareils sans effort, idéal pour alterner entre différents postes de travail.`}
							</p>
						</div>
						<div>
							<h3 className="font-semibold text-lg mb-2">
							...
							<p className="text-gray-700">
								...
								Son prix de {formatPrix(item.price.amazon.current)} offre un excellent rapport qualité-prix dans sa catégorie.
							</p>
						</div>
					</div>
				</section>
