...
type Hub = {
	...
	features: {
		...
		compatibiliteMac?: string;
		portsUsb: string;
		ethernet: string;
		audio?: string;
		lecteurCartes?: string;
		puissance: string;
		alimentationSupp?: string;
		technologie?: string;
	...
	ratings: {
		overall: number;
		portSelection: number;
		macCompatibility: number;
		buildQuality: number;
...
export default function HubUsbClient({ slug }: { slug: string }) {
	const id = slug;

	// Recherche dans hubs_stations
	const allProducts: Hub[] = [...HubData.hubs_stations] as Hub[];
		...
		return (
			<div className="min-h-screen flex items-center justify-center">
				<div className="text-center">
					...
					<Link href="/accessoires/hubUsb" className="bg-(--prim) text-white px-6 py-3 rounded-full hover:opacity-90 transition-all">
						Retour aux hubs USB
					</Link>
				</div>
			</div>
		...
		<>
			...
			<div className="section-bg text-white flex flex-col">
				...
				<div className="flex flex-wrap items-center text-base sm:text-xl mt-3 gap-0.5 xs:gap-1">
					...
					<Link href="/accessoires/hubUsb" className="hover:text-(--prim) transition-all duration-300">
						Hubs USB
					</Link>
					<i className="ri-arrow-right-wide-fill mt-1"></i>
					<span className="GolosText text-white truncate max-w-20 xs:max-w-30sm:max-w-50 lg:max-w-none">{product.name}</span>
				</div>
			</div>
					...
					<div className="w-full lg:w-1/2">
						...
						<div className="flex flex-wrap gap-2 mb-10 sm:mb-6">
							<span className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								<i className="ri-usb-line mr-1"></i>{product.features.connexionPc}
							</span>
							<span className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								<i className="ri-tv-line mr-1"></i>{product.features.sortiesVideo}
							</span>
							<span className="bg-blue-100 text-blue-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								<i className="ri-flashlight-line mr-1"></i>{product.features.puissance}
							</span>
						...
						<div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-6">
							...
							<div className="space-y-4 text-sm xs:text-base">
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Connexion PC</span>
									<span className="font-medium text-right max-w-[60%]">{product.features.connexionPc}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Sorties vidéo</span>
									<span className="font-medium text-right max-w-[60%]">{product.features.sortiesVideo}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Résolution max</span>
									<span className="font-medium text-right max-w-[60%]">{product.features.resolutionMax}</span>
								</div>
								{product.features.compatibiliteMac && (
									<div className="flex justify-between py-3 border-b border-gray-100">
										<span className="text-gray-500">Compatibilité Mac</span>
										<span className="font-medium text-right max-w-[60%]">{product.features.compatibiliteMac}</span>
									</div>
								)}
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Ports USB</span>
									<span className="font-medium text-right max-w-[60%]">{product.features.portsUsb}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Ethernet</span>
									<span className="font-medium text-right max-w-[60%]">{product.features.ethernet}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Puissance</span>
									<span className="font-medium text-right max-w-[60%]">{product.features.puissance}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
					...
					<div className="space-y-6">
						<div>
							<h3 className="font-semibold text-lg mb-2">
								De combien de ports USB ai-je besoin pour le télétravail ?
							</h3>
							<p className="text-gray-700">
								Pour un setup télétravail standard, comptez au minimum 4 à 6 ports USB
								(clavier, souris, webcam, casque, stockage externe). Le {product.name} propose
								{product.features.portsUsb} et {product.features.sortiesVideo} en sorties vidéo,
								ce qui couvre largement les besoins d&apos;un bureau moderne avec plusieurs écrans.
							</p>
						</div>
						<div>
							<h3 className="font-semibold text-lg mb-2">
								Quelle puissance de charge choisir pour un hub USB ?
							</h3>
							<p className="text-gray-700">
								La puissance Power Delivery est essentielle pour charger votre laptop via le hub.
								Le {product.name} délivre {product.features.puissance}, suffisant pour alimenter
								la plupart des ordinateurs portables tout en connectant vos périphériques.
								Pour un MacBook Pro, visez au minimum 60W de Power Delivery.
							</p>
						</div>
						<div>
							<h3 className="font-semibold text-lg mb-2">
							...
							<p className="text-gray-700">
								...
								Avec sa connexion {product.features.connexionPc} et sa garantie de {product.features.garantie},
								il offre un excellent rapport qualité-prix à {product.price.amazon.current}€
								pour les télétravailleurs qui ont besoin d&apos;une connectique fiable et complète.
							</p>
						</div>
					</div>
				</section>
