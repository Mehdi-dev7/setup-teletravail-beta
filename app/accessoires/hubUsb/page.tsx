...
export default function HubUsb() {
	return (
		<>
			<div className="section-bg text-white flex flex-col">
				<h1 className="text-4xl md:text-6xl xl:text-8xl text-center GolosText pt-10">Hubs & Stations</h1>
				<div className="flex items-center text-xl mt-3">
					...
					<h2 className="GolosText">Hubs USB</h2>
				</div>
			</div>
			...
			<div className="px-4 sm:px-[8%] lg:px-[12%] py-20">
				...
				<div className="max-w-4xl mb-16">
					...
					<h2 className="Sora text-4xl sm:text-5xl md:text-5xl 2xl:text-6xl  mb-5 leading-tight my-4 sm:my-5">
						Les meilleurs{" "}
						<span className="text-(--prim)">hubs USB et stations d&apos;accueil</span>
					</h2>
					<p className="md:text-lg text-base text-gray-600 leading-relaxed mb-4">
						Découvrez notre sélection des meilleurs hubs USB-C et stations d&apos;accueil
						pour étendre les capacités de votre laptop. Des{" "}
						<strong>stations Thunderbolt 4 premium</strong> aux{" "}
						<strong>hubs USB-C compacts</strong>, en passant par les{" "}
						<strong>docks DisplayLink multi-écrans</strong>.
					</p>
					<p className="text-lg text-gray-600 leading-relaxed">
						Que vous ayez besoin de connecter plusieurs écrans 4K à votre MacBook M1/M2/M3,
						de charger votre laptop en passthrough, ou simplement d&apos;ajouter des ports USB,
						vous trouverez ici la solution idéale.
					</p>
				</div>

				{/* Section Hubs et Stations */}
				<div className="mb-20">
						...
						<div className="flex flex-col flex-1">
							<div className="flex items-center justify-between">
								<h3 className="text-2xl md:text-4xl Sora font-semibold">
									Hubs & Stations d&apos;accueil
								</h3>
								<span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm">
									{HubData.hubs_stations.length} produits
								</span>
							</div>
							<span className="text-base md:text-lg text-gray-500 mt-1">
								Étendez les capacités de votre laptop
							</span>
						</div>
					</div>

					<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
						{HubData.hubs_stations.map((item, index) => (
							<Link href={`/accessoires/hubUsb/${item.id}`} key={item.id}>
									...
									<div className="p-5">
										...
										<div className="flex flex-wrap gap-2 mb-4">
											<span className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs">
												{item.features.connexionPc}
											</span>
											{item.features.puissance && (
												<span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs">
													{item.features.puissance}
												</span>
											)}
											{item.features.ethernet && (
