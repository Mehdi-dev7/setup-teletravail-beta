...
export default function ChaiseClient({ slug }: { slug: string }) {
	...
	// Recherche dans chaises_ergonomiques
	const allChaises: Chaise[] = [...ChaiseData.chaises_ergonomiques] as Chaise[];
	const chaise = allChaises.find((item) => String(item.id) === String(id));
		...
		return (
			<div className="min-h-screen flex items-center justify-center">
				<div className="text-center">
					...
					<h1 className="text-3xl GolosText mb-2">Chaise non trouvée</h1>
					<p className="text-gray-500 mb-6">La chaise que vous recherchez n&apos;existe pas.</p>
					<Link href="/chaises" className="bg-(--prim) text-white px-6 py-3 rounded-full hover:opacity-90 transition-all">
						Retour aux chaises
					</Link>
				</div>
			</div>
	...
	const imagesToShow = chaise.images && chaise.images.length > 0 ? chaise.images : [chaise.image];

	// Prix enseignes par défaut si non définis
	const prixEnseignes = chaise.prixEnseignes || [
		{ enseigne: "Amazon", prix: chaise.prix, url: `https://www.amazon.fr/dp/${chaise.amazonASIN}`, logo: "/logos/amazon.svg" },
		{ enseigne: "Cdiscount", prix: Math.round(chaise.prix * 1.02), url: "#", logo: "/logos/cdiscount.svg" },
		{ enseigne: "Fnac", prix: Math.round(chaise.prix * 1.05), url: "#", logo: "/logos/fnac.svg" },
	];
	...
	const jsonLd = {
		...
		image: chaise.images || [chaise.image],
		brand: {
			"@type": "Brand",
			name: chaise.marque,
		},
		...
		aggregateRating: {
			...
			ratingValue: chaise.note,
			reviewCount: chaise.nbAvis,
			bestRating: 5,
			worstRating: 1,
		},
	};
	...
	const faqJsonLd = {
		...
		mainEntity: [
			{
				"@type": "Question",
				name: "Comment régler une chaise ergonomique pour le télétravail ?",
				acceptedAnswer: {
					"@type": "Answer",
					text: `Pour un confort optimal, réglez la hauteur d'assise (${chaise.hauteurAssise}) pour que vos pieds soient à plat au sol. Le ${chaise.nom} dispose d'un ${chaise.supportLombaire} et d'accoudoirs ${chaise.accoudoirs} pour un soutien personnalisé.`,
				},
			},
			{
				"@type": "Question",
				name: `Quelle est l'inclinaison du ${chaise.nom} ?`,
				acceptedAnswer: {
					"@type": "Answer",
					text: `Le ${chaise.nom} offre une inclinaison de ${chaise.inclinaison}, avec un revêtement en ${chaise.revetement} et une capacité de charge de ${chaise.capaciteCharge} pour un confort durable tout au long de la journée.`,
				},
			},
			{
				"@type": "Question",
	...
	return (
		<>
			...
			<div className="section-bg text-white flex flex-col">
				<h1 className="text-4xl md:text-6xl xl:text-8xl text-center GolosText mt-10 md:mt-15">{chaise.marque}</h1>
				<div className="flex flex-wrap items-center text-base sm:text-xl mt-3 gap-0.5 xs:gap-1">
					...
					<Link href="/chaises" className="hover:text-(--prim) transition-all duration-300">
						Chaises
					</Link>
					<i className="ri-arrow-right-wide-fill mt-1"></i>
					<span className="GolosText text-white truncate max-w-20 xs:max-w-30 sm:max-w-50 lg:max-w-none">{chaise.nom}</span>
				</div>
			</div>
			...
			<div className="px-4 sm:px-[8%] lg:px-[10%] py-10 md:py-16">
				...
				<div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
					...
					<div className="w-full lg:w-1/2">
						{/* Marque et nom */}
						<p className="text-gray-500 text-lg mb-1">{chaise.marque}</p>
						<h1 className="text-3xl sm:text-4xl lg:text-5xl Sora mb-4">{chaise.nom}</h1>
						...
						<div className="flex items-center gap-3 mb-6">
							<div className="flex items-center">
								{[...Array(5)].map((_, i) => (
									<i
										key={i}
										className={`ri-star-${i < Math.floor(chaise.note) ? "fill" : "line"} text-lg sm:text-xl text-yellow-400`}
									></i>
								))}
							</div>
							<span className="text-sm sm:text-lg text-gray-600">
								{chaise.note}/5 <span className="text-gray-400">({chaise.nbAvis} avis)</span>
							</span>
						</div>
						...
						<div className="flex flex-wrap gap-2 mb-10 sm:mb-6">
							<span className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								<i className="ri-armchair-line mr-1"></i>{chaise.accoudoirs}
							</span>
							<span className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								{chaise.revetement.split(" ")[0]}
							</span>
							<span className="bg-gray-100 text-gray-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								<i className="ri-arrow-up-down-line mr-1"></i>{chaise.inclinaison.split(" ")[0]}
							</span>
							{chaise.reposePieds && (
								<span className="bg-blue-100 text-blue-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
									<i className="ri-footprint-line mr-1"></i>Repose-pieds
								</span>
							)}
							<span className="bg-green-100 text-green-700 px-3 py-2 rounded-lg text-xs md:text-sm font-medium">
								<i className="ri-shield-check-line mr-1"></i>{chaise.garantie}
							</span>
						</div>
						...
						<div className="bg-blue-50 rounded-2xl p-0 sm:p-4">
							...
							<p className="text-gray-700 text-sm sm:text-base leading-relaxed">{chaise.pourQui}</p>
						</div>
					</div>
				</div>
						...
						<div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-6">
							...
							<div className="space-y-4 text-sm xs:text-base">
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Revêtement</span>
									<span className="font-medium text-right max-w-[60%]">{chaise.revetement}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Support lombaire</span>
									<span className="font-medium text-right max-w-[60%]">{chaise.supportLombaire}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Accoudoirs</span>
									<span className="font-medium text-right max-w-[60%]">{chaise.accoudoirs}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Appui-tête</span>
									<span className="font-medium text-right max-w-[60%]">{chaise.appuiTete}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Repose-pieds</span>
									<span className="font-medium">{chaise.reposePieds ? "Oui" : "Non"}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Inclinaison</span>
									<span className="font-medium text-right max-w-[60%]">{chaise.inclinaison}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Hauteur assise</span>
									<span className="font-medium">{chaise.hauteurAssise}</span>
								</div>
								<div className="flex justify-between py-3 border-b border-gray-100">
									<span className="text-gray-500">Capacité charge</span>
									<span className="font-medium">{chaise.capaciteCharge}</span>
								</div>
								<div className="flex justify-between py-3">
									<span className="text-gray-500">Garantie</span>
									<span className="font-medium">{chaise.garantie}</span>
								</div>
							</div>
						</div>
					</div>
					...
					<div className="lg:col-span-2 space-y-6">
						...
						<div className="bg-green-50 rounded-2xl p-3 sm:p-6">
							...
							<ul className="space-y-3">
								{chaise.pointsForts.map((point, index) => (
						...
						<div className="bg-red-50 rounded-2xl p-3 sm:p-6">
							...
							<ul className="space-y-3">
								{chaise.pointsFaibles.map((point, index) => (
									<li key={index} className="flex items-start gap-3">
				...
				<div className="mt-12">
					<div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-6 md:p-8">
						...
						<p className="text-gray-700 text-base sm:text-lg leading-relaxed">{chaise.description}</p>
					</div>
				</div>
				...
				<div className="mt-12">
					<div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-6 md:p-8">
						...
						<div className="space-y-6">
							<div>
								<h3 className="text-lg font-semibold text-gray-900 mb-2">Comment régler une chaise ergonomique pour le télétravail ?</h3>
								<p className="text-gray-700 text-base leading-relaxed">
									Pour un confort optimal, réglez la hauteur d&apos;assise ({chaise.hauteurAssise}) pour que vos pieds soient à plat au sol. Le {chaise.nom} dispose d&apos;un {chaise.supportLombaire} et d&apos;accoudoirs {chaise.accoudoirs} pour un soutien personnalisé.
								</p>
							</div>
							<div>
								<h3 className="text-lg font-semibold text-gray-900 mb-2">Quelle est l&apos;inclinaison du {chaise.nom} ?</h3>
								<p className="text-gray-700 text-base leading-relaxed">
									Le {chaise.nom} offre une inclinaison de {chaise.inclinaison}, avec un revêtement en {chaise.revetement} et une capacité de charge de {chaise.capaciteCharge} pour un confort durable tout au long de la journée.
								</p>
							</div>
							<div>
