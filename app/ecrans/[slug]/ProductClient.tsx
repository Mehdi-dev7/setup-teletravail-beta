...
export default function EcranClient({ slug }: { slug: string }) {
	const id = slug;
	...
	const jsonLd = {
		...
		image: ecran.images || [ecran.image],
		brand: {
			"@type": "Brand",
			name: ecran.marque,
		},
		...
		aggregateRating: {
			...
			ratingValue: ecran.note,
			reviewCount: ecran.nbAvis,
			bestRating: 5,
			worstRating: 1,
		},
	};
	...
	const faqJsonLd = {
		...
		mainEntity: [
			{
				"@type": "Question",
				name: "Quelle résolution choisir pour un écran de télétravail ?",
				acceptedAnswer: {
					"@type": "Answer",
					text: `Pour le télétravail, un écran ${ecran.taille} avec une résolution ${ecran.resolution} et une dalle ${ecran.dalle} offre un confort visuel optimal pour travailler de longues heures.`,
				},
			},
			{
				"@type": "Question",
				name: `Quelles sont les connectiques du ${ecran.nom} ?`,
				acceptedAnswer: {
					"@type": "Answer",
					text: `Le ${ecran.nom} dispose de ${ecran.hdmi} port${ecran.hdmi > 1 ? "s" : ""} HDMI, ${ecran.displayport} port${ecran.displayport > 1 ? "s" : ""} DisplayPort${ecran.usbc ? `, un port USB-C${ecran.usbcWatt ? ` (${ecran.usbcWatt}W)` : ""}` : ""}. Il est compatible VESA ${ecran.vesa} pour un montage sur bras.`,
				},
			},
			{
				"@type": "Question",
				name: `Pourquoi choisir le ${ecran.nom} ?`,
				acceptedAnswer: {
					"@type": "Answer",
					text: `Le ${ecran.nom} se distingue par : ${ecran.pointsForts[0]}. Avec un temps de réponse de ${ecran.tempsReponse}, une fréquence de ${ecran.frequence} et un prix de ${formatPrix(ecran.prix)}, il offre un excellent rapport qualité-prix.`,
				},
			},
		],
	};
	...
	return (
		<>
			...
			<div className="section-bg text-white flex flex-col">
				<h1 className="text-4xl md:text-6xl xl:text-8xl text-center GolosText mt-10 md:mt-15">{ecran.marque}</h1>
				<div className="flex flex-wrap items-center text-base sm:text-xl mt-3 gap-0.5 xs:gap-1">
					...
					<i className="ri-arrow-right-wide-fill mt-1"></i>
					<span className="GolosText text-white truncate max-w-20 xs:max-w-30 sm:max-w-50 lg:max-w-none">{ecran.nom}</span>
				</div>
			</div>
				...
				<div className="mt-12">
					<div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-6 md:p-8">
						...
						<div className="space-y-6">
							<div>
								<h3 className="text-lg font-semibold text-gray-900 mb-2">Quelle résolution choisir pour un écran de télétravail ?</h3>
								<p className="text-gray-700 text-base leading-relaxed">
									Pour le télétravail, un écran {ecran.taille} avec une résolution {ecran.resolution} et une dalle {ecran.dalle} offre un confort visuel optimal pour travailler de longues heures. Une bonne résolution réduit la fatigue oculaire et améliore la productivité.
								</p>
							</div>
							<div>
								<h3 className="text-lg font-semibold text-gray-900 mb-2">Quelles sont les connectiques du {ecran.nom} ?</h3>
								<p className="text-gray-700 text-base leading-relaxed">
									Le {ecran.nom} dispose de {ecran.hdmi} port{ecran.hdmi > 1 ? "s" : ""} HDMI, {ecran.displayport} port{ecran.displayport > 1 ? "s" : ""} DisplayPort{ecran.usbc ? `, un port USB-C${ecran.usbcWatt ? ` (${ecran.usbcWatt}W)` : ""}` : ""}. Il est compatible VESA {ecran.vesa} pour un montage sur bras articulé.
								</p>
							</div>
							<div>
								<h3 className="text-lg font-semibold text-gray-900 mb-2">Pourquoi choisir le {ecran.nom} ?</h3>
								<p className="text-gray-700 text-base leading-relaxed">
									Le {ecran.nom} se distingue par : {ecran.pointsForts[0]}. Avec un temps de réponse de {ecran.tempsReponse}, une fréquence de {ecran.frequence} et un prix de {formatPrix(ecran.prix)}, il offre un excellent rapport qualité-prix pour le télétravail.
								</p>
							</div>
						</div>
					</div>
